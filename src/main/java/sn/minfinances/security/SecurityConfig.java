...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth.jdbcAuthentication()
			...
			.authoritiesByUsernameQuery("SELECT u.email as principal, r.nom as role FROM user u, role r, user_roles ur WHERE u.id = ur.users_id and r.id = ur.roles_id and u.email = ?")
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests().antMatchers("/accueil").hasAnyAuthority("ROLE_ADMIN", "ROLE_USER");
		http.authorizeRequests().antMatchers("/admin/users").hasAnyAuthority("ROLE_ADMIN");
		//gestion des droits
